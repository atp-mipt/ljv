...
:graphviz_code: ../../../../ljv/src/test/resources/graphviz
...
И так выглядит отрисовка графа объектов в памяти
--
...
[.notes]
--
Но если много памяти аллоцировалось под нагрузкой массив, а потом очередь очистилась,
